...
# ─── Pydantic schemas (exactly one definition) ─────────────────────────────────
...
async def send_message(
    ...
    # 3) Extract raw IV (bytes 9–24 of the decoded token)
...
async def get_conversation_summaries(
    ...
    for m in msgs:
        ...
        if partner.role == user.role:  # skip same-role
            continue
    ...
    q    = await db.execute(
        ...
        .where(
            or_(
                ...
                (Message.receiver_id == user.id) & (Message.sender_id   == partner_id)
            )
        )
